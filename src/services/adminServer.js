...
export const CreateNew = async (
  ...
  const response = await axios.post(
    `${process.env.REACT_APP_BACKEND}/api/v1/news/create`,
    {
      title,
      isChecked,
      show,
...
export const EditNew = async (
  ...
  const response = await axios.patch(
    `${process.env.REACT_APP_BACKEND}/api/v1/news/edit/${id}`,
    {
      id,
      title,
      description,
...
export const DeleteNew = async (id) => {
  const response = await axios.patch(
    `${process.env.REACT_APP_BACKEND}/api/v1/news/delete/${id}`,
    {
      id,
    }
  );
...
export const ShowNew = async () => {
  const response = await axios.get(
    `${process.env.REACT_APP_BACKEND}/api/v1/news/show`
  );
  return response.data;
};
